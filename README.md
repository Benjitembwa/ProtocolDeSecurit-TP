...
> Ne réutilisez jamais ces identifiants dans une application réelle.
...
Les identifiants MongoDB et autres secrets backend ne doivent jamais être placés dans une variable préfixée par `VITE_`.
...
| `npm start`        | Lance l’API et sert le frontend compilé      |
| `npm run db:check` | Vérifie MongoDB Atlas                        |
| `npm run seed`     | Initialise les données de démonstration      |
| `npm test`         | Lance les tests d’intégration et de sécurité |
| `npm run test:e2e` | Lance les tests navigateur                   |
| `npm run check`    | Compile puis exécute les tests API           |
...
sequenceDiagram
    ...
    DB-->>API: Utilisateur + rôle
    API-->>F: Authentification JWT

    U->>F: Lance une simulation
    F->>API: Requête simulation
    API->>DB: Modification logique des ressources
    API->>DB: Création de l'incident
    DB-->>API: État mis à jour
    API-->>F: Résultat simulation

    U->>F: Lance la récupération
    F->>API: Demande de restauration
    API->>DB: Vérification sauvegarde
    API->>DB: Restauration ressources
    DB-->>API: Ressources restaurées
    API-->>F: Incident résolu
```

---

# 🛡️ Flux de réponse à incident

```mermaid
flowchart TD
    A[État normal] --> B[Simulation d'un incident]
    B --> C[Détection]
    C --> D[Incident créé]
    D --> E[Confinement]
    E --> F[Analyse]
    F --> G[Sélection d'une sauvegarde]
    G --> H[Vérification SHA-256]
    H --> I{Intégrité valide ?}

    I -->|Non| J[Choisir une autre sauvegarde]
    J --> G

    I -->|Oui| K[Restauration]
    K --> L[Contrôle d'intégrité]
    L --> M[Réactivation des services]
    M --> N[Clôture de l'incident]
    N --> O[Calcul du score de résilience]
```
...
Ce projet met en pratique plusieurs concepts étudiés en sécurité informatique et sécurité réseau :
